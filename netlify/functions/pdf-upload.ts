...
export default async (req: Request, context: Context) => {
    ...
    try {
        ...
        if (!contentType.includes("multipart/form-data")) {
            return new Response(JSON.stringify({ message: "Content type must be multipart/form-data" }), {
                status: 400,
        ...
        while ((match = displayRegex.exec(fullText)) !== null) {
            ...
            if (!seenFormulas.has(equation) && equation.length < 150) {
                ...
                const context = fullText.substring(contextStart, contextEnd).replace(/\s+/g, " ").trim();
