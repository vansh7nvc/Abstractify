    ...
    try {
        ...
        if (!geminiKey) {
            ...
            papers.forEach((p, idx) => {
                ...
                let stance = "neutral";
                if (text.includes("improve") || text.includes("better") || text.includes("outperform") || text.includes("significantly")) {
                    stance = "supports";
                    supports++;
                } else if (text.includes("limitation") || text.includes("fail") || text.includes("degrade") || text.includes("worse")) {
                    stance = "contradicts";
                    contradicts++;
                } else {
                    neutral++;
            ...
            const score = (supports + contradicts) > 0 ? (supports / (supports + contradicts)) * 100 : 0;
            ...
            return new Response(JSON.stringify({
                ...
                summaryText: "[Local Fallback Mode] The consensus summary is generated based on keyword matching because no Gemini API key is configured. Please enter a key in the settings for advanced AI analysis.",
...
`;
        ...
        const result = JSON.parse(cleanedText) as { stances: Record<string, string>; summary: string };
